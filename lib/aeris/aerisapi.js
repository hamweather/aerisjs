...
function(aeris) {
  ...
  /**
   * Fetch fire data from the AerisAPI.
   *
   * @param {Object} opt_params Search parameters.
   * @return {aeris.Promise} A promise to fetch fire data.
   */
  aeris.AerisAPI.prototype.getFireData = function(opt_params) {
    var params;
    ...
    this.fetchBatch([{
      ...
      // Because this is a batch request, we receive an array of responses
      // even though there is only a single response
      // Only return the first response
      // to mimic a single-endpoint request
      promise.resolve(data.responses[0]);
    }).fail(function(error) {
  ...
  /**
   * Fetch lightning data from the AerisAPI.
   *
   * @param {Array.<Array.<Number, Number>, Array.<Number, Number>>} bounds
   *     The NE to SW bounds of the map to get lightning data for.
   * @param {Object} opt_params Search parameters.
   * @return {aeris.Promise} A promise to fetch lightning data.
   */
  aeris.AerisAPI.prototype.getLightningData = function(bounds, opt_params) {
    var params;
    var promise = new aeris.Promise();
    opt_params || (opt_params = {});

    params = aeris.extend({
      p: bounds[0][0] + ',' + bounds[1][1] + ',' +
  ...
  /**
   * Fetch storm report data from the AerisAPI.
   *
   * @param {Array.<Array.<Number, Number>, Array.<Number, Number>>} bounds
   *     The NE to SW bounds of the map to get storm report data for.
   * @param {Object} opt_params Search parameters.
   * @return {aeris.Promise} A promise to fetch storm report data.
   */
  aeris.AerisAPI.prototype.getStormReportData = function(bounds, opt_params) {
    var params;
    var promise = new aeris.Promise();
    opt_params || (opt_params = {});
  ...
  /**
   * Fetch earthquake data from the AerisAPI.
   *
   * @param {Array.<Array.<Number, Number>, Array.<Number, Number>>} bounds
   *     The NE to SW bounds of the map to get earthquake data for.
   * @param {Object} opt_params Search parameters.
   * @return {aeris.Promise} A promise to fetch earthquake data.
   */
  aeris.AerisAPI.prototype.getEarthquakeData = function(bounds, opt_params) {
    var params;
    var promise = new aeris.Promise();
    opt_params || (opt_params = {});
   ...
   */
  aeris.AerisAPI.prototype.getSunMoon = function(latLon, opt_globalParams) {
    var promise = new aeris.Promise();
    ...
    this.fetchBatch(endpoints, globalParams).done(function(data) {
      ...
      throw new Error('Unable to get sun/moon data: ' + error.description);
    });
  ...
  aeris.AerisAPI.prototype.getWeather = function(latLon, opt_globalParams) {
    ...
    this.fetchBatch(endpoints, globalParams).done(function(data) {
      ...
      throw new Error('Unable to get weather data: ' + error.description);
    });
