define([
  ...
  /**
   * Render all the markers for a given bounds on the map.
   *
   * @param {Array.<Array.<number, number>, Array.<number, number>>} bounds
   *     The bounds for the markers to render.
   */
  aeris.maps.MarkerCollection.prototype.renderMarkers =
      function(bounds, opt_refresh) {
    var refresh = typeof opt_refresh !== 'undefined' ? opt_refresh : true;

    this.markerData_ = refresh ? this.getMarkerData(bounds) : this.markerData_;
    this.markerData_.
      done(this.onFetchMarkerSuccess_, this).
      fail(this.onFetchMarkerError_, this);
  };
   ...
   * @private
   */
  aeris.maps.MarkerCollection.prototype.renderMarkers_ = function(opt_markers) {
   ...
   * @abstract
   * @param {Array.<Array.<number, number>, Array.<number, number>>} bounds
   *     The bounds for the marker data.
   * @return {aeris.Promise)
   * @protected
   */
  aeris.maps.MarkerCollection.prototype.getMarkerData = aeris.abstractMethod;
